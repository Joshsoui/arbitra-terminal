...
export function normalizeGoogleTrendsRow(row: GoogleTrendsRow): RawObservation[] {
  ...
  const base: Omit<RawObservation, "signalType" | "value" | "unit"> = {
    ...
    metadata: {
      ...
      percentGain: row.percentGain ?? null,
    },
  };
...
WITH latest_partition AS (
  ...
  SELECT
    ...
    percent_gain,
    ROW_NUMBER() OVER (
      PARTITION BY country_code, region_code, term, week
      ORDER BY refresh_date DESC
    ) AS row_num
  FROM \`bigquery-public-data.google_trends.international_top_rising_terms\`
  WHERE refresh_date = (SELECT refresh_date FROM latest_partition)
    AND country_code IN ('GB', 'DE', 'NL')
)
SELECT
  country_code,
  region_code,
...
FROM ranked
WHERE row_num = 1
ORDER BY country_code, term, week;
`;
...
WITH latest_partition AS (
  ...
  SELECT
    ...
    AVG(score) AS score,
    MIN(rank) AS rank,
    AVG(percent_gain) AS percent_gain,
    COUNT(DISTINCT dma_id) AS dma_count
  FROM dma_rows
  GROUP BY term, week
)
SELECT term, week, score, rank, percent_gain, dma_count
FROM national
ORDER BY term, week;
`;
